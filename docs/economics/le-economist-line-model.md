...
For a deadline decision, the scenario layer provides the sanctioned fallback rather than refusing to calculate: the fixed shared facility cost is allocated by modeled kitchen hours, while another documented shared production pool may be allocated by modeled direct production labor hours. Every fallback result is labeled modeled and paired with order-count and revenue-share sensitivities. The decision must report the range and whether the line ranking or recommendation reverses. Direct joins replace—not validate—the fallback when they arrive.
