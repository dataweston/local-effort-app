...
- Initialize a Next.js 14 application in `apps/web` with shadcn/ui and Tailwind.
- Bootstrap NestJS services in `services/api`, `services/dispatcher`, and `services/billing`.
- Create Node-based workers for `services/worker` and `services/labeler` with BullMQ integration.

## 3. Data model
- Define the Prisma schema under `packages/db`, covering organizations, programs, orders, batches, invoices, loyalty, referrals, incidents, delivery jobs, and webhooks.
- Seed baseline SKUs (pizza and sandwiches) sourced from the Midwest.

## 4. Contracts and tooling
- Author the OpenAPI 3.1 specification in `packages/contracts/openapi.yaml` for all `/v1` endpoints and webhook payloads.
...
## 7. Infrastructure
- Produce Dockerfiles per service, a Helm chart under `packages/infra`, and GitHub Actions workflows for CI/CD.
