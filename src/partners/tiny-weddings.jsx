...
export default function TinyWeddingsProxy() {
  const configuredUrl = import.meta.env.VITE_TINY_WEDDINGS_URL;
  ...
  if (!targetUrl) {
    return (
      <div className="max-w-3xl space-y-4 rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
        <h2 className="text-3xl font-semibold">Tiny Weddings</h2>
        <p>
          The Tiny Weddings app runs as a standalone service.
        </p>
        <ol className="list-decimal space-y-1 pl-6 text-sm text-neutral-700">
          <li>Start the service locally or deploy it.</li>
          <li>
            Set <code>VITE_TINY_WEDDINGS_URL</code> to the deployed URL (or use the dev port{' '}
