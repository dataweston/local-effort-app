      ...
      {expanded && data && (
        <div className="border-t px-4 py-4" style={{ borderColor: 'var(--color-border-default)' }}>
          ...
          <div className="mt-3 space-y-1 text-[11px]" style={{ color: 'var(--color-text-muted)' }}>
            <p>Square revenue uses {data.sources.square.recurringSeriesCount} live non-overridden monthly invoice series detected from actual invoice history, plus future one-off invoices already scheduled. Meal-prep recipients with owner-confirmed schedules are excluded from this Square projection, and {data.sources.square.churnedSeriesCount || 0} churned series worth {money(data.sources.square.churnedMonthlyCents)}/mo are excluded because their newest invoice is stale, canceled, or refunded.</p>
            <p>Happy Monday is forecast a week at a time: each week counts the larger of orders already placed or the planner Sunday run-rate card. The live trailing {data.sources.happyMonday.runRateWindowWeeks || 8}-week average of actual order weeks is {money(data.sources.happyMonday.runRateWeeklyCents)}/week, booked in the week it is earned — Happy Monday settles in batched invoices roughly one to two weeks later, so month boundaries are approximate. {money(data.sources.happyMonday.unpaidOrderCents)} of already-delivered orders is still unpaid and is not counted as future cash. Local Budget inventory and operating projections use the average of six complete actual months.</p>
