...
- **Implementation Sketch**:
  - Extend ecipes OpenSearch mapping with location as nested keyword (already populated via curation).
...
- **Implementation Sketch**:
  ...
  - Extend ecipes documents with location.centroid (lat/lon) when spatial hints are available; fall back to county centroids.
  - Integrate eact-leaflet in the search page’s sidebar, highlighting polygons for selected facets and plotting available items.
...
- **Implementation Sketch**:
  - Update the FastAPI search response to include highlight fragments from OpenSearch (highlight parameter on 	itle/description).
