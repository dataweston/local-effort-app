...
module.exports = async function handler(req, res) {
  ...
  const expectedState = cookies.sq_oauth_state;
  if (!state || !expectedState || state !== expectedState) {
    console.error('[HappyMonday] Square OAuth state mismatch');
    return res.redirect('/partners/happy-monday?sq_error=invalid_state');
  }
  // Clear the state cookie
  res.setHeader('Set-Cookie', 'sq_oauth_state=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0');
